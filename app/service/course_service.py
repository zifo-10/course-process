...
from app.models.processing_models import ProcessedParagraph, SimplifyResults, QuizResults, ProcessedParagraphWithSkills
...
def get_skills(paragraph_list: List[ProcessedParagraph]) -> List[ProcessedParagraphWithSkills]:
    try:
...
async def simplify_paragraph_v1(paragraphs: List[ProcessedParagraphWithSkills]) -> List[SimplifyResults]:
    logger.info("Starting paragraph simplification...")

    async def simplify_single(paragraph: ProcessedParagraphWithSkills) -> SimplifyResults:
        try:
