...
1. **Short Paragraph**: 
   If the paragraph is short enough (less than 150 words), return it exactly as is in **Arabic Language88, without any changes. Ensure that the paragraph remains unaltered and is presented in its original form.

2. **Long Paragraph**: 
   If the paragraph is too long (more than 150 words), break it into chunks that make sense semantically in Arabic. Each chunk should:
   - Be coherent, logically organized, and easy to understand on its own.
   - Retain all relevant information and meaning from the original paragraph.
   - Not lose the overall context.
   - Not modify any words, punctuation, or structure of the original paragraph; simply divide it into digestible parts.
   - Ensure that all chunks are in Arabic Language.
   - Ensure that chunks are not too short, ideally maintaining a reasonable length that preserves the meaning. Avoid chunks that are too brief or fragmented unless absolutely necessary.
   - **Most importantly**: Ensure that each chunk is **exactly from the original video**, without adding or changing any part of the content.
...
4. **Response Format**:
   - If the paragraph is short, return it as is in Arabic.
   - If the paragraph is long, return the chunks as a list of individual sections in Arabic. Ensure each section is self-contained and retains the original wording.
   - If the section is meaningless, return an empty list.

**Important**: Do not change any words, phrases, or punctuation in the paragraph. The goal is to maintain the **exact content** in Arabic, just divided into manageable sections for long paragraphs or return an empty list if the content lacks meaning. Ensure the chunks are not too short and that each one is logically meaningful.
"""

simplify_prompt_ar = """
You will receive an Arabic paragraph. Your job is to fix it, extract important words or phrases, and rewrite it in three versions that get simpler and more detailed in **Arabic**.
...
Step 1: Fix the Paragraph
- Correct spelling mistakes in **Arabic
...
# Version 3 – Simplest and Longest (in **Arabic):
- Audience: A child or beginner.
- Use easy, friendly language (like “Imagine…”).
- For each important word/phrase, use a comparison with at least two examples.
- Add more detail (a small story, extra context, playful tone).
...
- Make sure the versions get longer: Original < V1 < V2 < V3.
- Output this:
  1. Fixed Original Paragraph in **Arabic**
  2. Version 1 in **Arabic**
  3. Version 2 in **Arabic**
  4. Version 3 in **Arabic**

⚠️ If the paragraph is part of a course or a specific topic, such as the introduction to a training course, do **not** alter or simplify it. It must be kept intact without any changes, and the output should be in the **Arabic Language**.
"""

question_generation_prompt_ar = """
You are an expert **Arabic content developer. Analyze the given video script and generate assessment questions based strictly and only on its content.
...
1. **Question Generation**:
    - Create exactly **2 independent questions** (MCQs and True/False).
    - Each must include a **clear, factual answer**.
    - Questions should be **standalone**, written in **grammatically correct Arabic**.
    - Focus on **facts, statistics, and key ideas**—avoid assumptions.
...
📌 Final Instructions:
    ...
    - Use concise, factual choices for MCQs.
    - If the question is True/False, **do not begin it with "True or False:"** — just ask the question directly.
    - All question must be in **Arabic Language**.
